...
def min_variability(df, model, training_budget, include_all_classes=False, dataset='animals'):
    ...
    print("unique targets min variability per class: ", len(set(unique_targets)))
    print('samples - min variability per class: ', len(set(sampled_question_ids)))
    print('all_samples - min variability per class: ', len(all_ids))
