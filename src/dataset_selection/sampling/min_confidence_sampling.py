...
def min_confidence(df, model, training_budget, include_all_classes=False, dataset='animals'):
    ...
    for label in unique_targets:
        ...
        df_filtered = df_filtered.sort_values(by=['confidence'], ascending=True)
    ...
    if include_all_classes == True:
        ...
        save_path = 'src/dataset_selection/sampling/samples/'+model+'/'+dataset+'/min_confidence/include_all_classes/seed_'+str(args.seed)+'/budget_'+str(training_budget)+'.pkl'
    else:
        save_path = 'src/dataset_selection/sampling/samples/'+model+'/'+dataset+'/min_confidence/seed_'+str(args.seed)+'/budget_'+str(training_budget)+'.pkl'
    ...
    print("unique targets min confidence per class: ", len(set(unique_targets)))
    print('samples - min confidence per class: ', len(set(sampled_question_ids)))
